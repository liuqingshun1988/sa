...
app.post('/api/shipments', auth, (req, res) => {
  const d = req.body;
  const id = uid();
  db.prepare(`INSERT INTO shipments (id,sup_id,sup_name,mer_id,name,qty,items,batch,produce_date,date,report,report_ref,report_url,status,ts)
    VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`).run(
    id, req.user.id, req.user.name, d.merId, d.name, d.qty||'',
    JSON.stringify(d.items||[]), d.batch||'', d.produceDate||'', d.date||'',
...
app.put('/api/shipments/:id/confirm', auth, (req, res) => {
  ...
  const ledgerId = uid();
  db.prepare(`INSERT INTO ledger (id,mer_id,name,cat,qty,supplier,sup_id,date,produce_date,batch,secret,report,report_url,report_ref,ts)
    VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`).run(
    ledgerId, req.user.id, s.name, s.cat||'其他', s.qty, s.sup_name, s.sup_id,
    new Date().toISOString().slice(0,10), s.produce_date||'', s.batch||'', 0,
    s.report||'', s.report_url||'', autoReportRef, Date.now()
  );
