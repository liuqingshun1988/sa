...
app.put('/api/shipments/:id/confirm', auth, (req, res) => {
  ...
  if(s.mfr_id) {
    ...
    if(merUser) {
      try {
      db.prepare(`UPDATE mfr_shipments SET status='done' WHERE id=(SELECT id FROM mfr_shipments WHERE mfr_id=? AND to_name=? AND status='pending' LIMIT 1)`).run(s.mfr_id, merUser.name);
    } catch(e) {}
    }
  }
...
app.put('/api/shipments/:id/status', auth, (req, res) => {
  ...
  // 拒收时同步厂家端 mfr_shipments 状态
  if(req.body.status === 'reject' && s.mfr_id) {
    try {
      const merUser = db.prepare('SELECT name FROM users WHERE id=?').get(req.user.id);
      if(merUser) {
        db.prepare(`UPDATE mfr_shipments SET status='reject' WHERE id=(SELECT id FROM mfr_shipments WHERE mfr_id=? AND to_name=? AND status='pending' LIMIT 1)`).run(s.mfr_id, merUser.name);
      }
    } catch(e) {}
  }
  res.json({ ok: true });
...
app.get('/api/sup-mfrs', auth, (req, res) => {
  const rows = db.prepare(`
    SELECT s.*, u.name as mfr_name
    FROM sup_mfrs s LEFT JOIN users u ON s.mfr_id=u.id
    WHERE s.mfr_id=? OR s.sup_id=?
...
app.get('/api/mfr-mers', auth, (req, res) => {
  const rows = db.prepare(`
    SELECT m.*, u.name as mer_name
    FROM mfr_mers m LEFT JOIN users u ON m.mer_id=u.id
    WHERE m.mfr_id=? OR m.mer_id=?
  `).all(req.user.id, req.user.id);
  res.json(rows);
});
...
app.post('/api/mfr-shipments', auth, (req, res) => {
  ...
  // 如果收货方是已注册供应商，通知其待收货
  const toUser = d.toId ? db.prepare('SELECT * FROM users WHERE id=?').get(d.toId) : db.prepare('SELECT * FROM users WHERE name=? AND role="sup"').get(d.to||'');
  if(toUser && toUser.role === 'sup') {
    // 在 mfr_shipments 里 to_id 已记录，供应商端通过 API 可查到
    db.prepare('UPDATE mfr_shipments SET to_id=? WHERE id=?').run(toUser.id, id);
  }
  res.json({ id });
});

// 供应商查看厂家发来的待收货
app.get('/api/sup-pending-receives', auth, (req, res) => {
  try { db.prepare('ALTER TABLE mfr_shipments ADD COLUMN to_id TEXT DEFAULT ""').run(); } catch(e) {}
  const rows = db.prepare('SELECT * FROM mfr_shipments WHERE to_id=? ORDER BY ts DESC').all(req.user.id);
  res.json(rows);
});

// 供应商确认收货（厂家发来的）
app.post('/api/sup-pending-receives/:id/confirm', auth, (req, res) => {
  const s = db.prepare('SELECT * FROM mfr_shipments WHERE id=? AND to_id=?').get(req.params.id, req.user.id);
  if(!s) return res.status(404).json({ error: 'not found' });
  db.prepare('UPDATE mfr_shipments SET status="done" WHERE id=?').run(req.params.id);
  res.json({ ok: true });
});

// 供应商拒收（厂家发来的）
app.post('/api/sup-pending-receives/:id/reject', auth, (req, res) => {
  const s = db.prepare('SELECT * FROM mfr_shipments WHERE id=? AND to_id=?').get(req.params.id, req.user.id);
  if(!s) return res.status(404).json({ error: 'not found' });
  db.prepare('UPDATE mfr_shipments SET status="reject" WHERE id=?').run(req.params.id);
  res.json({ ok: true });
});
// 厂家修改发货记录（收货前）
...
app.get('/api/pub/:merId', (req, res) => {
  ...
  // 收集所有涉及的厂家ID（从台账mfr_id字段）
  const mfrIds = [...new Set(ledger.map(l => l.mfr_id).filter(Boolean))];
  const mfrQuals = {};
  mfrIds.forEach(mfrId => {
    mfrQuals[mfrId] = db.prepare("SELECT * FROM quals WHERE owner_id=? AND (type='营业执照' OR type='食品生产许可证' OR type='食品生产许可证（SC）')").all(mfrId);
  });
  res.json({ mer, quals, ledger, suppliers, mfrQuals });
});
