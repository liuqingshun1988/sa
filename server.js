...
app.post('/api/shipments', auth, (req, res) => {
  const d = req.body;
  const id = uid();
  db.prepare(`INSERT INTO shipments (id,sup_id,sup_name,mer_id,name,qty,items,batch,produce_date,expiry,date,report,report_ref,report_url,status,ts)
    VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`).run(
...
app.put('/api/shipments/:id/confirm', auth, (req, res) => {
  ...
  const ledgerId = uid();
  db.prepare(`INSERT INTO ledger (id,mer_id,name,cat,qty,supplier,sup_id,date,produce_date,batch,expiry,secret,report,report_url,report_ref,ts)
    VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`).run(
